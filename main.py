...
# методы GET  запрашивает данные(read)
# метод POST отправляет данные на сервер(submit)
# метод PUT принудительно заменяет все на сервере из контекста запроса(заменить)
# метод DELETE  удаляет данные
# метод PATСH частично изменяет данные